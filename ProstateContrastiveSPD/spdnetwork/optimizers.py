...
class MixOptimizer():
    ...
    def __init__(self, parameters, optimizer=torch.optim.SGD, lr=1e-2, lr_others = None, learning_scheduler=None, scheduler_kwargs=None, *args, **kwargs):
        ...
        #Defining the optimizers for each Riemannian constraint.
        ...
        #Defining the optimizer for the other parameters.
        if learning_scheduler is not None:
            logging.info("Using learning scheduler")
            self.optim = optimizer(self.other_parameters, lr, *args, **kwargs)
            self.scheduler = learning_scheduler(self.optim, **scheduler_kwargs)
        elif lr_others is None:
            logging.info("Using same learning rate for all parameters, i.e, lr_others = lr")
            self.optim = optimizer(self.other_parameters, lr, *args, **kwargs)
            self.scheduler = None
        else:
            logging.info(f"Using different learning rates for different parameters, lr = {lr} AND lr_others = {lr_others}")
            self.optim = optimizer(self.other_parameters, lr_others, *args, **kwargs)
            self.scheduler = None
        
    def step(self):
        # Doing the step for each Riemannian optimizer.
        ...
        if self.optim is not None:
            self.optim.step() # Doing the step for the other parameters.
