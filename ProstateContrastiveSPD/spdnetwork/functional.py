...
def init_bimap_parameter(W):
    ...
    if ni>=no:
        for i in range(ho):
            for j in range(hi):
                # print(W.device)
                v = th.empty(ni,ni, dtype = W.dtype,device=W.device).uniform_(0.,1.) # generate a uniform random matrix with range [0, 1]
                vv = th.svd(v.matmul(v.t()))[0][:,:no] #Take the firts no eigenvetors of v\cdot v^T.
                W.data[i,j] = vv
    else:
        for i in range(ho):
            for j in range(hi):
                v = th.empty(no,no, dtype = W.dtype,device=W.device).uniform_(0.,1.) # generate a uniform random matrix with range [0, 1]
                vv = th.svd(v.matmul(v.t()))[0][:,:ni] #Take the firts no eigenvetors of v\cdot v^T.
                W.data[i,j] = th.transpose(vv,0,1)
...
def init_bimap_parameter_identity(W):
    ...
    if ni>=no:
        ...
        for i in range(ho):
            for j in range(hi):
                W.data[i,j]=th.eye(no,ni)
                
        
    # print(f'NN W BiMap device en init bimap parameters:', self._W.device)
...
#     print(f'X:', X.device)
    ...
    for co in range(channels_out):
        P[:,co,:,:]=sum([bimap(X[:,ci,:,:],W[co,ci,:,:]) for ci in range(channels_in)]) #Each output channel is the sum of the last spd matrices projected with different matrices
    return P
...
def modeig_forward(P,op,eig_mode='svd',param=None):
    ...
    for i in range(batch_size):
        for j in range(channels):
            if(eig_mode=='eig'):
                s,U[i,j]=th.eig(P[i,j],True); S[i,j]=s[:,0] #S autovalores (b,ch,n), U autovectores (b,ch,n,n)
            elif(eig_mode=='svd'):
...
def recov_forward(P):
    '''
    Forward function of Recov operation
    Input P: (batch_size,channels) SPD matrices of size (n,n)
    Output X: (batch_size,channels) modified symmetric matrices of size (n,n)
    '''
    batch_size,channels,n,n=P.shape #batch size,channel depth,dimension
    P_new = th.zeros_like(P,device=P.device)
...
def modeig_backward(dx,U,S,S_fn,op,param=None):
    ...
    S: autovalores (batch, ch, n)
    F_fn: autovalores con la funcion aplicada
    Input U: (batch_size,channels) SPD matrices of size (n,n)
    Output X: (batch_size,channels) modified symmetric matrices of size (n,n)
    '''
    # if __debug__:
    #     import pydevd
    ...
    #diagonaliza, fn_deriv derivada de la operacion op \Sigma^{-1}
    S_fn_deriv = BatchDiag(op.fn_deriv(S,param)) 
    SS=S[...,None].repeat(1,1,1,S.shape[-1]) #[...,None] agrega un axis. Se copia el tensor S (batch, ch, n,n) 
    ...
    return dp #L U.T dx U, L deberia ser S_fn_deriv 
...
def cov_pool(f,reg_mode='mle'):
    ...
    """
    #print('{f.shape}')
    bs,n,T=f.shape #LE QUITÉ EL BS
    X=f.matmul(f.transpose(-1,-2))/(T-1) # se arma una matriz SPD no necesariamente una COV
    #print(f'In functional cov_pool X shape: {X.shape}')
    if(reg_mode=='mle'):
        ret=X
    ...
    if(len(ret.shape)==3):
        return ret[:,None,:,:]
    print(f'In functional cov_pool ret shape: {ret.shape}')
    return ret
...
def cov_pool_mu(f,reg_mode):
    ...
    alpha=1
    bs,n,T=f.shape
    mu=f.mean(-1,True); f=f-mu #Se resta la media al X de features aplanadas X (n,T)
    X=f.matmul(f.transpose(-1,-2))/(T-1) #Se calcula la matriz de covarianza
